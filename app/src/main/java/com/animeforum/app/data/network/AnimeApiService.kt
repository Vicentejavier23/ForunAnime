...
class AnimeApiService {
  suspend fun fetchRandomAnimeSuggestion(): AnimeSuggestion? = withContext(Dispatchers.IO) {
    ...
    try {
      ...
      if (code in 200..299) {
        ...
        if (title.isNotBlank()) AnimeSuggestion(
          title = title,
  ...
  private fun translateToSpanish(text: String?): String? {
    ...
    val chunkSize = 450
    val parts = text.chunked(chunkSize)
    ...
    for (part in parts) {
      ...
      translatedParts.add(fragment ?: part)
...
annotation class AnimeSuggestion(val title: String, val synopsis: String)
